...
Текущая версия: **v0.8.77** (выпущена, ежедневный рабочий инструмент / daily driver).
Файл: `Multi-LLM-Prompter-v0_8_77.ps1` (~10 440 строк, ~430 КБ).
...
-> v0.8.77 (clip хедер-чекбокса ИСПРАВЛЕН + визуально проверен off-screen рендером: Top + Margin 0,5,0,0).
...
`Multi-LLM-Prompter-v0_8_77.ps1` - ~10 440 строк. PS 5.1, исходник только ASCII (Unicode только как
...
- v0.8.77: clip хедер-чекбокса ИСПРАВЛЕН и ВИЗУАЛЬНО ПРОВЕРЕН. Слепой цикл разорван рендером
  реализованного грида Tasks off-screen в PNG (RenderTargetBitmap) - реально посмотрел + измерил пиксели
  (см. технику "GUI render-verify" в разделе 8). Находка: контент DataGridColumnHeader имеет ~5-7px
  отрицательный верхний сдвиг, и дефолтный презентер игнорирует `VerticalContentAlignment=Center`,
  поэтому бокс 16x16 прижимался к верху и обрезался. Фикс: `VerticalAlignment="Top"` + `Margin="0,5,0,0"`
  на bare-чекбоксе -> бокс на y~12.5-28.5 внутри заголовка 36px, полностью виден и выровнен с подписями
  колонок (подтверждено 4x композитным рендером). НЕ Grid-обёртка (она исчезла в v0.8.75). Только
  XAML/раскладка; замороженные функции, контракт судьи, маршрутизация, расчёт стоимости не тронуты.
...
- GUI render-verify (v0.8.77): когда визуальную/пиксельную проблему нельзя оценить по одному XAML
  (обрезка, выравнивание, наложение), НЕ гадать вслепую. Отрендерить реализованный контрол off-screen в
  PNG и ПОСМОТРЕТЬ: в STA-runspace `XamlReader.Load` извлечённого из AST `$GuiXamlTemplate`, `$win.Show()`
  на далёком off-screen `Left/Top`, **выбрать вкладку с контролом** (TabControl виртуализирует - у
  невыбранной вкладки ActualHeight 0), задать фиктивный `ItemsSource` для DataGrid, прокачать диспетчер
  до `ApplicationIdle` + `UpdateLayout()` (заголовки DataGrid генерируются отложенно), затем
  `RenderTargetBitmap.Render(control)` в PNG (dpi = 96*N даёт зум Nx) и прочитать PNG. `TransformToAncestor`
  даёт точные смещения ребёнок-vs-родитель для численного измерения обрезки. Так выяснилось, что презентер
  DataGridColumnHeader игнорирует `VerticalContentAlignment=Center`.
