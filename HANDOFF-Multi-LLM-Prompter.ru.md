...
Текущая версия: **v0.8.59** (выпущена, ежедневный рабочий инструмент / daily driver).
Файл: `Multi-LLM-Prompter-v0_8_59.ps1` (~9 540 строк, ~416 КБ).
...
**ВЛИТ в `main`** (merge-коммит `8fcde6d`). Всё после него - v0.8.56, v0.8.57, v0.8.58, v0.8.59 -
закоммичено прямо в `main` (по одному коммиту на версию, согласно правилу версионирования). Текущий
HEAD `main` - коммит v0.8.59.
...
`Multi-LLM-Prompter-v0_8_59.ps1` - ~9 540 строк. PS 5.1, исходник только ASCII (Unicode только как
...
- v0.8.59: кнопка Run авто-подгоняет ширину под подпись. Динамическая подпись "Run (N tasks) - est.
  ...
  ДАЛЕЕ (нужен живой GUI-проход): expander "Advanced settings", сворачиваемая quick-start карточка,
  empty-states для вкладок/очереди.
