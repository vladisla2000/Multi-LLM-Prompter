...
Текущая версия: **v0.8.72** (выпущена, ежедневный рабочий инструмент / daily driver).
Файл: `Multi-LLM-Prompter-v0_8_72.ps1` (~10 390 строк, ~430 КБ).
...
`Multi-LLM-Prompter-v0_8_72.ps1` - ~10 390 строк. PS 5.1, исходник только ASCII (Unicode только как
...
- v0.8.72: полировка ДОВЕРИЯ первого запуска по живому ревью v0.8.61. (a) Карточка Cost справа теперь
  ...
  (новые поля стоимости + хедер-чекбокс находятся); harness PASS=48/0. ОТКРЫТО из ревью (нужны фиксы):
  #5 согласованность отображения типа задачи, #7 дублирующая подпись "Full Answer" (кнопка панели vs
  вкладка), #9 тупик/устаревший контент Improved Prompt, #10 жаргон пресета "SingleAD"; плюс живой
  ре-чек #3/#4/#6 (время/счётчик/токены).
